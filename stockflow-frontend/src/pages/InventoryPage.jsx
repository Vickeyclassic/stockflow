...
const emptyFilters = { productId: '', transactionType: '', dateFrom: '', dateTo: '' };
export default function InventoryPage() {
  ...
  useEffect(() => {
    ...
    const params = { page, size: 20, ...Object.fromEntries(Object.entries(applied).filter(([, value]) => value)) };
  ...
  return <>
    ...
    <section className="inventory-panel"><h2>Transaction history</h2><form className="filters" onSubmit={e => { e.preventDefault(); setPage(0); setApplied({ ...filters }); }}>
    ...
    </form>{!error && <MovementTable rows={history.content} />}<Pager page={page} setPage={setPage} data={history} busy={loading} /></section>
  </>;
}
