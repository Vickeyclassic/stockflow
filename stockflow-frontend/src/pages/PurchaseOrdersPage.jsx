...
const blank = () => ({ orderNumber: '', supplierId: '', orderDate: today(), notes: '', items: [newItem()] });
const emptyFilters = { orderNumber: '', supplierId: '', status: '', dateFrom: '', dateTo: '' };

function OrderForm({ order, suppliers, products, onClose, onSaved }) {
  const [values, setValues] = useState(() => order ? { ...order, supplierId: order.supplier.id } : blank());
  ...
  function changeItem(index, key, value) {
    setValues(v => ({ ...v, items: v.items.map((item, i) => {
      ...
      if (key === 'productId') return { ...item, productId: value, unitPrice: products.find(p => String(p.id) === value)?.costPrice ?? '' };
      return { ...item, [key]: value };
    }) }));
  }
  async function submit(event) {
    event.preventDefault(); setBusy(true); setError(null);
    const body = { orderNumber: values.orderNumber.trim(), supplierId: Number(values.supplierId), orderDate: values.orderDate, notes: values.notes?.trim() || null,
      items: values.items.map(i => ({ productId: Number(i.productId), quantity: Number(i.quantity), unitPrice: String(i.unitPrice) })) };
    try { const { data } = order ? await api.put(`/api/purchase-orders/${order.id}`, body) : await api.post('/api/purchase-orders', body); onSaved(data); }
    catch (e) { setError(e); } finally { setBusy(false); }
  }
  return <Modal title={order ? 'Edit draft order' : 'Create purchase order'} onClose={onClose} busy={busy}>
    <form onSubmit={submit}><ErrorNotice error={error} /><fieldset disabled={busy}><div className="form-grid">
      ...
      {field('supplierId', 'Supplier', { required: true, options: [{ value: '', label: 'Select supplier' }, ...suppliers.map(c => ({ value: c.id, label: c.name }))] })}
    ...
    <p className="muted">The saved total is calculated by StockFlow. Stock increases only when an ordered purchase order is received.</p>
...
export default function PurchaseOrdersPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([api.get('/api/purchase-orders', { params, signal: controller.signal }), api.get('/api/suppliers', { signal: controller.signal }), loadProducts(controller.signal)])
      .then(([o, c, p]) => { setData(o.data); setSuppliers(c.data); setProducts(p); })
  ...
  async function view(id) {
    setError(null);
    try { const { data } = await api.get(`/api/purchase-orders/${id}`); setStatusError(null); setDetail(data); } catch (e) { setError(e); }
  }
  async function changeStatus(status) {
    setBusy(true); setStatusError(null);
    try { const { data } = await api.patch(`/api/purchase-orders/${detail.id}/status`, { status }); setDetail(data); setRevision(r => r + 1); setMessage(`Order ${data.orderNumber}: ${status}.`); }
  ...
  return <>
    <div className="page-heading"><div><p className="eyebrow">SUPPLIER ORDERS</p><h1>Purchase orders</h1><p>Create drafts, place orders, and receive items into stock.</p></div><button className="primary" disabled={loading || !!error} onClick={() => setEditing({})}>+ Create order</button></div>
    <form className="filters" onSubmit={e => { e.preventDefault(); setApplied({ ...filters }); setPage(0); }}>
      {filter('orderNumber', 'Order number')}
      {filter('supplierId', 'Supplier', { options: [{ value: '', label: 'All suppliers' }, ...suppliers.map(c => ({ value: c.id, label: c.name }))] })}
    ...
    {editing && <OrderForm order={editing.id ? editing : null} suppliers={suppliers} products={products} onClose={() => setEditing(null)} onSaved={o => { setEditing(null); setDetail(o); setStatusError(null); setRevision(r => r + 1); setMessage('Draft saved.'); }} />}
    {detail && <Modal title={`Order ${detail.orderNumber}`} busy={busy} onClose={() => setDetail(null)}>
      <p>{detail.supplier.name} · {detail.orderDate} · <strong>{detail.status}</strong></p><p className="muted">Created {dateTime(detail.createdAt)}</p><p>{detail.notes}</p>
      ...
      {detail.status === 'ORDERED' && <p>Receiving adds all items to stock and is final.</p>}
      {detail.status === 'RECEIVED' && <p className="muted">Inventory history reference: PURCHASE_ORDER #{detail.id}</p>}
      <div className="form-actions">
        {detail.status === 'DRAFT' && <><button disabled={busy} onClick={() => { setEditing(detail); setDetail(null); }}>Edit draft</button><button className="primary" disabled={busy} onClick={() => changeStatus('ORDERED')}>Place order</button></>}
        {detail.status === 'ORDERED' && <button className="primary" disabled={busy} onClick={() => changeStatus('RECEIVED')}>{busy ? 'Receiving…' : 'Receive order'}</button>}
        {['DRAFT', 'ORDERED'].includes(detail.status) && <button disabled={busy} onClick={() => changeStatus('CANCELLED')}>Cancel order</button>}
      </div>
    </Modal>}
  </>;
}
