...
const currentPage = () => [...pages, 'users'].includes(window.location.hash.slice(1)) ? window.location.hash.slice(1) : 'dashboard';
...
function AuthenticatedApp() {
  ...
  return <div className="shell">
    ...
    <nav aria-label="Main navigation">{(user.role === 'ADMIN' ? [...pages, 'users'] : pages).map(name => <a key={name} href={`#${name}`} aria-current={page === name ? 'page' : undefined}>{pageLabel(name)}</a>)}<span>V1.1</span></nav>
    <main>{page === 'users' ? (user.role === 'ADMIN' ? <UsersPage /> : <DashboardPage />) : page === 'dashboard' ? <DashboardPage /> : page === 'purchase-orders' ? <PurchaseOrdersPage /> : page === 'sales-orders' ? <SalesOrdersPage /> : page === 'products' ? <ProductsPage /> : page === 'inventory' ? <InventoryPage /> : ['purchase-receipts', 'sales-issues'].includes(page) ? <DocumentsPage key={page} purchase={page === 'purchase-receipts'} /> : <CatalogPage key={page} kind={page} />}</main>
