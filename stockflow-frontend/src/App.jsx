...
const pages = ['products', 'categories', 'suppliers', 'inventory', 'purchase-receipts', 'sales-issues'];
...
export default function App() {
  ...
  return <div className="shell">
    ...
    <nav aria-label="Main navigation">{pages.map(name => <a key={name} href={`#${name}`} aria-current={page === name ? 'page' : undefined}>{pageLabel(name)}</a>)}<span>PHASE 03</span></nav>
    <main>{page === 'products' ? <ProductsPage /> : page === 'inventory' ? <InventoryPage /> : ['purchase-receipts', 'sales-issues'].includes(page) ? <DocumentsPage key={page} purchase={page === 'purchase-receipts'} /> : <CatalogPage key={page} kind={page} />}</main>
