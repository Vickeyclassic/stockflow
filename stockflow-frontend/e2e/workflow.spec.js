...
test('ADMIN purchase-to-sale workflow, exports, invoice, inventory and logout', async ({ page }) => {
  ...
  const purchase = `PO-${suffix}`, sale = `SO-${suffix}`;
  ...
  async function stockIs(quantity) {
    ...
    await page.getByLabel('Search SKU', { exact: true }).fill(sku);
  ...
  await test.step('Login as ADMIN and create catalog records', async () => {
    ...
    await dialog().getByLabel('Supplier', { exact: true }).selectOption({ label: supplier });
  ...
  await test.step('Create and receive a purchase; stock increases', async () => {
    ...
    await dialog().getByLabel('Supplier', { exact: true }).selectOption({ label: supplier });
    await dialog().getByLabel('Item 1 product').selectOption({ label: `${sku} · ${product} (stock 0)` });
    ...
    await page.getByLabel('Order number', { exact: true }).fill(purchase);
    await page.getByRole('button', { name: 'Apply filters', exact: true }).click();
  ...
  await test.step('Create a customer and fulfill a sales order', async () => {
    ...
    await dialog().getByLabel('Address', { exact: true }).fill('Demo Street\nDemo City');
    ...
    await dialog().getByLabel('Customer', { exact: true }).selectOption({ label: customer });
    await dialog().getByLabel('Item 1 product').selectOption({ label: `${sku} · ${product} (stock 10)` });
  ...
  await test.step('Preview and print the invoice; export the filtered sales order', async () => {
    ...
    await dialog().getByRole('button', { name: 'Close dialog' }).click();
    await page.getByLabel('Order number', { exact: true }).fill(sale);
    await page.getByRole('button', { name: 'Apply filters', exact: true }).click();
  ...
  await test.step('Filter movement history, export it, and log out', async () => {
    ...
    await history.getByLabel('History product').selectOption({ label: `${sku} · ${product} (stock 7)` });
    await history.getByLabel('History type').selectOption('STOCK_OUT');
    await history.getByLabel('Reference type', { exact: true }).selectOption('SALES_ORDER');
    ...
    await history.getByLabel('Rows per page').selectOption('10');
